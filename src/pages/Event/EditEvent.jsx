...
const EditEvent = () => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        const formattedDate = `${eventDate.getFullYear()}-${String(eventDate.getMonth() + 1).padStart(2, "0")}-${String(eventDate.getDate()).padStart(2, "0")}T${String(
          eventDate.getHours()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://kevent-server.onrender.com/events/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    <div className="edit-event-container">
      ...
      <form onSubmit={handleSubmit} className="edit-event-form">
        ...
        <div className="form-group">
          ...
          <input type="datetime-local" id="date" name="date" value={eventData.date} onChange={handleChange} required />
        </div>
        <div className="form-group">
        ...
        <div className="form-group">
          <label htmlFor="description">Description</label>
          <input type="text" id="department" name="Department" value={eventData.Department} onChange={handleChange} required />
        </div>

        <div className="form-group">
          <label htmlFor="contactNumber">Contact Number</label>
          <input type="text" id="contactNumber" name="contactNumber" value={eventData.contactNumber} onChange={handleChange} required />
        </div>
        <div className="form-group">
          <label htmlFor="capacity">Capacity</label>
          <input type="text" id="capacity" name="capacity" value={eventData.capacity} onChange={handleChange} required />
        </div>
        <div className="form-group">
          <label htmlFor="price">Price</label>
          <input type="text" id="price" name="Price" value={eventData.Price} onChange={handleChange} required />
        </div>
